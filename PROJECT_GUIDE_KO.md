...
| 브라우저 `localStorage` | 작은 메모지 | 편집자의 마지막 활동 시각처럼 작은 세션 보조값만 저장한다. |
...
- `test.html`: 미국 서부 현재 시각을 2027년 1월 25일 12:00로 고정해 DAY 5와 약 46% 진행 상태를 보여 준다.
...
8. 서버 저장본을 여행별 `IndexedDB` 공개 캐시에도 저장한다.
...
네트워크 장애 등으로 서버 저장본을 읽지 못하면 브라우저 `IndexedDB`에 남은 최근 공개 캐시를 사용하려고 한다. 캐시도 없다면 `trip-config.js`에서 만든 기본 내용을 그대로 보여 준다. 캐시가 오래되었으면 화면에 이를 알린다.
...
보기 모드에서 체크하거나 해제하면 `requireEditor('checklist')`가 편집자 세션을 확인한다. 인증을 취소하면 방금 바꾼 체크를 원래 상태로 되돌리고, 인증에 성공하면 저장되지 않은 초안으로 표시한다. 이후 나타나는 `변경사항 저장`을 눌러야 Supabase 최신본과 새 버전에 반영되므로 새로고침 후에도 유지된다.
...
각 일정 카드의 편집 도구에서는 사진, Google Maps 링크, 주의사항을 추가할 수 있다. 주의사항은 한 일정에 여러 개 둘 수 있으며 각각 내용을 수정하거나 삭제할 수 있다. 추가·수정·삭제는 실행 취소/다시 실행 기록과 저장본 차이 비교에 포함된다.

PREP에서는 체크리스트 항목을 추가·수정·삭제하고 위·아래 버튼으로 순서를 바꿀 수 있다. 모바일에서 실수하기 쉬운 드래그 대신 명시적인 이동 버튼을 사용한다. 새 항목은 실제 `새 체크 항목` 문자열을 넣지 않고 빈 편집 영역에 안내 문구만 표시하므로, 입력을 시작하면 안내가 자동으로 사라진다.
...
따라서 아무것도 수정하지 않고 편집 완료를 누르면 확인창 없이 바로 종료할 수 있다.

### 9.5 편집 완료

변경이 있으면 세 가지 선택이 나타난다.

- `저장하고 종료`: Supabase에 새 버전과 최신본을 기록하고 편집을 끝낸다.
- `계속 편집`: 대화상자를 닫고 편집을 이어 간다.
- `변경사항 폐기`: 마지막 저장본으로 화면을 되돌리고 편집을 끝낸다.

### 9.6 저장 버튼

저장 UI는 두 경로로 나뉜다.

- 일정·PREP 구조 편집: 상단 버튼이 `편집 완료`로 바뀌고, 완료 확인창에서 저장하거나 계속 편집하거나 폐기한다.
- 체크 완료 여부·가계부 빠른 변경: 보기 모드에서 변경이 생겼을 때만 `변경사항 저장` 버튼이 나타난다.

변경이 없을 때는 저장 버튼을 비활성 상태로 계속 보여 주지 않고 숨긴다. 두 경로 모두 최종적으로 같은 `persistState()`와 `save_trip_document` RPC를 사용하므로 저장본과 버전 관리 방식은 같다.
...
버전 관리 화면은 처음에 ID, 저장 메모, 용량, 저장 시각 같은 메타데이터만 요청한다. 전체 상태 본문은 사용자가 비교·미리보기·복원을 선택한 버전만 지연 로딩하고 브라우저 메모리에 잠시 캐시한다. 버전이 많아도 첫 목록 요청이 과도하게 커지는 것을 막는다.

`미리보기`를 누르면 현재 화면을 임시로 선택한 과거 버전으로 바꿔 보여 준다. 미리보기를 닫으면 원래 화면으로 돌아간다.

### 11.3 복원

과거 버전을 복원할 때 과거 기록을 잘라내거나 삭제하지 않는다. 선택한 과거 내용을 **새 저장 버전으로 다시 저장**한다. 이 방식은 복원 전 현재 상태도 버전 이력에 남을 가능성을 유지하고, 이력의 흐름을 단순하게 만든다.

저장할 때 선택적으로 짧은 메모를 남길 수 있어 버전 목록에서 변경 목적을 구분할 수 있다.

### 11.4 동시 저장 충돌

브라우저는 공개본을 읽을 때 `revision`도 함께 기억한다. 다른 사용자가 먼저 저장하면 서버 revision이 달라지므로, 이전 공개본을 기준으로 만든 내 초안을 무조건 덮어쓰지 않고 충돌 대화상자를 표시한다. 서버 최신본 보기, 내 초안 유지, 차이 비교 후 재적용 중 하나를 명시적으로 선택한다.
...
### 12.3 기존 항목 수정과 표시 순서

각 가계부 행의 `수정` 버튼을 누르면 날짜, 카테고리, 항목명, 금액, 통화를 한 번에 바꿀 수 있다. 전체 일정 편집 모드를 켤 필요는 없지만 추가와 마찬가지로 실제 변경에는 유효한 편집자 인증이 필요하다.

가계부는 날짜순으로 자동 정렬하지 않고 **추가된 순서**를 기본 표시 순서로 사용한다. 각 행의 위·아래 버튼으로 항목을 이동하면 `expenses` 배열 자체의 순서가 바뀐다. 이 순서도 초안과 저장본 비교에 포함되며, 최종 `저장` 후 다른 사용자에게 공개된다.

입력 영역은 가계부 내용의 위쪽에 있어 새 항목을 추가한 뒤 아래의 등록순 목록에서 확인한다.

### 12.4 통화와 환율

각 항목마다 KRW 또는 USD를 선택한다. 페이지 전체 통화를 바꾸는 것이 아니라 해당 금액이 어떤 통화인지 저장한다.

가계부 탭에서는 공통 `편집 시작` 버튼 대신 `가계부 설정` 버튼이 보인다. 이 설정 화면에서 환율과 카테고리를 함께 관리하므로, 일정 전체를 편집 가능한 상태로 바꾸지 않는다.

환율은 `가계부 설정`에서 직접 입력하며 자동 금융 API를 사용하지 않는다. 입력값은 초안 상태 변수에 즉시 반영되어 목록이나 합계를 다시 그려도 기본값으로 돌아가지 않는다. USD 금액을 원화 환산할 때 다음 개념으로 계산한다.
...
가계부 상단의 예산은 원화 기준이다. 예산을 설정하면 사용액, 남은 금액 또는 초과액과 진행률을 보여 준다. 카테고리와 항목명 검색으로 목록을 필터링할 수 있으며, CSV 내보내기는 현재 필터 결과를 등록 순서대로 저장한다. 스프레드시트에서 수식으로 오인할 수 있는 `=`, `+`, `-`, `@`로 시작하는 값은 안전하게 이스케이프한다.

### 12.5 카테고리

`가계부 설정`에서 카테고리를 추가하거나 이름과 아이콘을 바꾸고 삭제할 수 있다. 화면 본문을 서버 저장본으로 다시 그린 뒤에도 동작이 끊기지 않도록 카테고리와 환율 조작은 문서 단위 이벤트 처리로 연결한다.

카테고리를 삭제하면 그 카테고리를 사용하던 가계부 항목 자체를 삭제하지 않는다. 해당 항목들의 카테고리 ID를 시스템 카테고리인 `삭제된 카테고리`로 바꾼다. 지출 기록을 잃지 않기 위한 처리다.

### 12.6 삭제 확인

가계부 항목 삭제는 복구가 필요한 실수가 되기 쉬우므로 확인 과정을 거친다. 삭제 후에도 아직 서버 저장 전이라면 마지막 저장본으로 초기화하거나 변경사항을 폐기할 수 있다.
...
따라서 사진을 추가한 뒤 폐기하거나 자동 로그아웃되면 서버에 불필요한 객체를 남기지 않는다. 업로드 중에는 진행률과 취소 UI를 제공하며, 저장이 성공하기 전까지 공개 문서는 바뀌지 않는다.
...
지도 캡처는 위치를 빠르게 확인하기 위한 비클릭 이미지이고, 실제 길 찾기는 별도의 Google Maps 링크를 사용한다. 정적 이미지 URL에도 내용 해시가 포함되어 이미지가 바뀔 때만 새 URL을 받도록 브라우저 캐시를 활용한다.
...
- 편집 도구
- 로그인/버전/알림 대화상자
- 가계부 입력·수정·삭제·순서 변경 UI
- 체크박스 조작

이 파일은 편집본으로 다시 가져오기 위한 파일이 아니다. 생성 시 가능한 사진은 썸네일과 큰 보기용 이미지를 각각 Base64로 포함한다. 다운로드하지 못한 사진은 온라인 URL을 유지할 수 있고, 지도 링크를 실제로 열 때는 인터넷이 필요하다.
...
| 편집 기능 | 있음 | 없음 |
| 사진 포함 | Base64로 파일 내부 포함 | 가능한 사진은 Base64로 포함, 실패분은 온라인 URL 유지 |
| Supabase 연결 | 없음 | 없음 |
| 온라인 원본으로 가져오기 | 가능 | 불가능 |
| 저장 결과 | 새 작업본 파일 다운로드 | 해당 없음 |
| 파일 크기 | 사진 수에 따라 매우 큼 | 사진 수에 따라 커질 수 있음 |
...
기존 서버에는 앱 배포 전에 `migrations/20260920_photo_jpeg_fallback.sql`을 실행한다. 버킷 허용 MIME과 업로드 경로 정책만 바꾸며, 기존 사진·여행 데이터·편집자 매핑은 유지한다.
...
### 21.1 `IndexedDB`와 `localStorage`

`IndexedDB`는 큰 구조화 데이터를 비동기로 보관한다. 이 앱은 최근 공개본, 온라인·오프라인 초안과 초안 작성 시 기준 revision을 여행별로 저장한다. 페이지를 새로 열었을 때 서버 공개본과 비교한 뒤 복구 여부를 묻는다. `localStorage`는 편집자의 마지막 활동 시각 같은 작은 세션 보조값에만 사용한다.

두 브라우저 저장소는 다음 이유로 공식 백업이 아니다.
...
이 캐시는 `IndexedDB`·`localStorage`와 다른 브라우저의 네트워크 캐시다.
...
| 체크 완료 여부 변경 | 공개 웹페이지 PREP 보기 모드 | 로그인 후 `변경사항 저장` | Supabase |
| 체크리스트 문구·순서 변경 | 공개 웹페이지 편집 모드 | `편집 완료`에서 저장 | Supabase |
| 가계부 추가·수정·삭제·순서 변경 | 공개 웹페이지 가계부 | 로그인 후 `변경사항 저장` | Supabase |
| 가계부 환율·카테고리 변경 | 공개 웹페이지 `가계부 설정` | `변경사항 저장` | Supabase |
| 사진 추가 | 공개 웹페이지 편집 모드 | 초안 확인 후 `저장` | 저장 시 Storage + Database |
...
- `trip_editors`에 그 여행 ID와 로그인 계정 UUID가 연결되어 있는가
...
- 편집 시작/완료/폐기/저장이 의도대로 동작하는가
- 체크 완료 여부를 바꾸면 인증 후 `변경사항 저장`이 나타나고, 취소 시 원래 체크로 돌아가는가
- 체크리스트 추가 항목의 안내 문구와 위·아래 순서 이동이 모바일에서도 자연스러운가
- 저장하지 않은 내용이 다른 브라우저에 보이지 않는가
- 저장한 뒤 다른 브라우저나 시크릿 창에서 보이는가
- 가계부 합계와 환율 계산이 맞는가
- 가계부 설정에서 카테고리 추가와 환율 변경 후 화면을 다시 그려도 초안 값이 유지되는가
- 미국 서부 날짜가 여행 전·중·후 경계에서 PREP·해당 DAY·가계부를 올바르게 선택하는가
- 서울·미국 서부 시계와 서머타임 시차가 올바른가
...
- 초안 복구 기능을 서버 백업으로 오해하지 않는다. 중요한 변경은 반드시 저장하거나 오프라인 작업본으로 내보낸다.
...
같은 Supabase 프로젝트를 재사용할 경우 공통 표와 함수는 공유할 수 있다. 업데이트된 `supabase_setup.sql`의 Storage 정책은 업로드 경로 첫 폴더와 `trip_editors.trip_id`를 비교하므로 여행 ID가 달라도 같은 버킷을 안전하게 나누어 쓴다. 이전 SQL을 사용한 기존 프로젝트는 웹앱 배포 전에 현재 `supabase_setup.sql`을 한 번 다시 실행하고, 이후 새 여행마다 `register_trip.sql`을 실행해 여행 ID와 편집자를 연결한다.
