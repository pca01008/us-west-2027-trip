# US West 2027 Trip

2027년 1월 21일~31일 라스베이거스와 로스앤젤레스 여행을 위한 공유 일정 페이지입니다.

## 구성

- `index.html`: 여행 일정, 체크리스트, 경비를 확인·편집하는 공통 웹앱
- `trip-config.js`: 현재 여행의 제목, 기간, 시간대, 기본 일정과 체크리스트
- `trip-config.template.js`: 다른 여행을 시작할 때 복사하는 설정 예시
- `supabase_setup.sql`: 모든 여행이 공유하는 실시간 저장, 버전 관리, 사진 Storage와 편집 권한 설정
- `register_trip.sql`: Supabase에 여행 ID와 편집자 계정을 연결하는 등록문
- `migrations/20260920_photo_jpeg_fallback.sql`: 기존 서버의 JPEG 사진 업로드 허용 설정. 앱 배포 전에 Supabase SQL Editor에서 실행한다.
- `scripts/`, `tests/`: 설정·HTML·SQL 정적 검사와 회귀 테스트
- `.github/workflows/validate.yml`: push와 pull request 때 자동으로 실행되는 검증
- `NEW_TRIP_GUIDE_KO.md`: 다른 여행으로 재사용하는 순서와 설정값 설명
- `.gitattributes`: Windows와 WSL 환경의 줄바꿈을 LF로 통일하는 규칙

## 사용 방법

사진 업로드 브라우저 검증은 `node scripts/check-photos-browser.mjs [playwright/index.mjs]`로 실행한다. 설치된 Chrome에서 실제 이미지 변환과 PNG 대체 반환 상황, 초안 저장 경로를 검사한다. 저장소는 테스트 대역을 사용하며 운영 서버에는 요청하지 않는다. 아이폰 실기기 검증은 별도다.

`index.html`을 브라우저로 열면 일정을 볼 수 있습니다. 누구나 열람할 수 있으며, 편집은 설정된 Supabase 편집자 계정으로 로그인한 경우에만 가능합니다.

- 편집 내용은 브라우저 IndexedDB 초안으로 유지되며, 확인 후 `저장`해야 공개 페이지에 반영됩니다. 새로고침이나 탭 종료 뒤에도 복구를 제안합니다.
- 여행 전에는 PREP, 여행 중에는 미국 서부 현지 날짜의 일정, 여행 후에는 가계부가 첫 화면으로 열립니다.
- 헤더에는 서울과 미국 서부의 현재 시각 및 자동 계산된 시차가 표시됩니다.
- 편집 중에는 실행 취소와 다시 실행을 사용할 수 있습니다.
- 로그인 세션은 편집 완료 후에도 유지됩니다. 마지막 활동 후 30분이 지나면 자동 로그아웃되며, 1분 전 경고에 응답하지 않으면 초안이 폐기됩니다.
- 체크 완료 여부와 가계부 내역은 보기 모드에서 빠르게 바꿀 수 있으며, 변경이 생겼을 때만 `변경사항 저장` 버튼이 나타납니다. 유효한 편집자 세션이 없으면 먼저 인증합니다.
- 체크리스트 항목은 편집 모드에서 추가·수정·삭제하고 위·아래 버튼으로 순서를 변경할 수 있습니다. 새 항목의 안내 문구는 실제 값이 아니므로 바로 입력하면 됩니다.
- 일정은 시간·세로선·마커·카드로 이어지는 타임라인이며, 편집 모드에서 일정별 사진과 HTTPS 지도 링크를 첨부할 수 있습니다.
- 새 일정의 제목·장소/메모·주의사항은 안내 문구를 지우지 않고 바로 입력합니다. 안내는 저장되지 않으며, 보기 화면에서는 빈 제목을 `제목 없음`으로 표시하고 빈 메모·주의사항은 숨깁니다. 기존에 저장한 내용은 그대로 유지합니다. 문자 시간(미정 등)과 사진 설명은 편집창을 열면 전체 선택되어 바로 바꿀 수 있습니다.
- 일별 일정의 편집 모드에서 `일정 시간 추가` 또는 카드의 `시간 설정`을 누르면 시작 시간과 선택적인 종료 시간을 따로 입력합니다. 흐린 `09:00` 예시 위에 숫자 4자리를 입력하면 시·분 자리만 덮어쓰며 콜론은 자동으로 건너뜁니다. 예시는 실제 값이 아니므로 네 자리를 모두 입력해야 하며, 종료 시간을 비우면 시작만, 둘 다 비우면 미정으로 저장합니다. 기존 시각도 숫자를 덮어쓰거나 선택한 부분만 수정할 수 있고, 삭제하면 해당 자리의 예시가 다시 보입니다. `오전·오후·미정 등` 방식에서는 자유롭게 시간대를 적을 수 있습니다. KST 등 시간대 표기는 별도로 유지하며 시작·종료에 같은 기준을 적용합니다. 종료가 시작보다 이르면 다음 날 종료로 안내합니다. 적용 전 취소하면 일정은 바뀌지 않고, 적용한 변경은 실행 취소·다시 실행이 가능합니다.
- `시간표`를 누르면 현재 불러온 일정을 날짜×시간으로 비교하는 보기 전용 화면이 열립니다. `1일`과 `전체` 보기를 제공하며 PC는 전체, 모바일은 1일 보기가 기본입니다. 날짜 이동은 1일 보기에서 사용하며, 전체 보기를 거쳐 돌아와도 선택했던 날짜가 유지됩니다. 일정 카드를 누르면 설명·주의사항·사진·지도 등 원래 상세 정보가 열립니다.
- `전체` 시간표는 모든 날짜 열과 시간축을 보여주는 `한눈에 보기`입니다. 넓은 화면에서는 한 화면에 압축하고, 800px 이하에서는 날짜 열의 너비를 확보해 좌우로 스크롤합니다. 작은 화면에서도 일정 이름과 시각을 표시하며 왼쪽 시간축은 고정됩니다. 상단의 시간 미정·종일·다른 시간대 일정은 일부 이름과 개수로 요약합니다. 일정을 누르면 상세 내용이 열리고, 날짜 제목을 누르면 해당 날짜의 1일 보기로 이동합니다.
- 시간표의 점선 카드는 시작 시각만 있는 일정입니다. 종료 시간을 추정하지 않으며, `09:00~11:00`처럼 구간이 명시된 경우에만 길이를 시간에 비례해 표시합니다. 겹치는 카드는 나란히 배치하고, 자정을 넘는 구간은 다음 날짜에 이어 표시합니다.
- `오전`, `미정`, `종일` 등 정확한 시각이 없는 일정과 KST 등 목적지 시간대와 다른 일정은 상단에 원래 표기로 모읍니다. 종류는 일정명과 보조 태그의 키워드로 자동 분류하며, 새 일정이나 이름을 수정한 일정도 시간표를 열 때 다시 분류합니다. 관광은 초록, 식사는 주황, 이동은 파랑, 숙소·휴식은 보라, 분류되지 않은 일정은 회색입니다. 예를 들어 `박물관 관람`, `점심 식사`, `공항 이동`은 해당 색상이 적용되지만 키워드가 없는 `새 일정`이나 장소명만 있는 일정은 기타로 표시될 수 있습니다. 1일 보기 하단에는 그날 등록된 체크인·숙박과 이동 내역을 요약합니다. 소요 시간이 없는 이동은 미정으로 표시합니다.
- 시간표 보기·날짜 이동은 여행 데이터나 초안을 변경하지 않습니다. 편집은 기존 일별 일정에서 진행하며, 인쇄·공유용 사본은 기존 일별 일정 형식으로 내보냅니다. 오프라인 작업본에서는 시간표도 사용할 수 있습니다.
- 일별 일정의 편집 모드에서 카드의 `일정 분류`를 선택할 수 있습니다. 기본값 `자동 (현재 분류)`은 제목·보조 태그를 따르며, 관광·식사·이동·숙소·휴식·기타를 직접 선택하면 해당 분류와 색상을 우선 적용합니다. 수동 선택은 제목을 바꿔도 유지되고, `자동`으로 되돌리면 현재 제목으로 다시 분류합니다. 분류 선택은 일정과 함께 저장·복원되고 실행 취소·다시 실행 및 오프라인 작업본에서도 유지되며, 버전·불러오기 비교에 분류 변경이 표시됩니다.
- 지도는 장소명과 외부 지도 URL을 저장하는 방식이므로, 오프라인 파일에서도 링크는 보이지만 실제 지도를 열 때는 인터넷 연결이 필요합니다.
- 사진 개수에는 제한이 없습니다. 추가한 사진은 우선 초안에만 보관되고 최종 저장할 때 업로드됩니다. 업로드 전에 WebP 원본(최대 1600px)과 썸네일(최대 480px)로 압축되며, 화면에서는 썸네일을 지연 로드합니다. 개별 원본 파일은 20MB까지 선택할 수 있습니다.
- 압축된 사진의 SHA-256 해시를 파일 경로로 사용하므로 동일한 사진은 같은 URL을 재사용합니다. 새 사진은 브라우저 캐시를 1년간 유지하고, 사진 내용이 바뀌면 새 URL을 생성해 변경된 파일만 다시 받습니다.
- 가계부는 보기 모드에서도 날짜·카테고리·항목·금액·통화를 추가·수정·삭제할 수 있습니다. 실제 변경 시 유효한 편집자 세션을 확인하며, `변경사항 저장` 전까지는 공개되지 않습니다.
- 가계부 탭의 `가계부 설정`에서 수동 환율과 카테고리를 관리합니다. 카테고리를 삭제하면 연결된 내역은 `삭제된 카테고리`로 이동합니다.
- 가계부는 예산 진행률, 카테고리·검색 필터, KRW/USD 합계, 원화 환산 합계와 카테고리별 합계를 보여주며 현재 목록을 안전한 CSV로 내보낼 수 있습니다.
- 저장본은 메모와 함께 최근 50개까지 보관됩니다. `버전 관리`는 목록에서 본문을 한꺼번에 내려받지 않고 선택한 버전만 불러와 비교·미리보기·복원합니다.
- `내보내기`에서 목적에 따라 `오프라인 작업본` 또는 `인쇄·공유용 사본`을 선택합니다.
- `오프라인 작업본`은 사진까지 파일 안에 포함하며 인터넷 없이 편집할 수 있습니다. 사진이 많아 100MB를 넘으면 다운로드 전에 경고합니다.
- 오프라인 작업본에서 편집한 뒤 저장하면 새 작업본이 내려받아집니다. 온라인 페이지의 `편집본 불러오기`는 최신 공개본과 달라진 일정을 먼저 보여주며, 확인한 내용은 초안으로만 적용됩니다. 별도로 `저장`해야 공개됩니다.
- `인쇄·공유용 사본`은 편집 기능과 Supabase 연결을 제거한 읽기 전용 스냅샷입니다. 가능한 사진은 파일에 포함하며, 포함하지 못한 사진과 지도 링크를 열 때만 인터넷 연결이 필요합니다.

## Supabase 초기 설정

1. Supabase Authentication에서 편집자 이메일 계정을 생성합니다.
2. SQL Editor에서 `supabase_setup.sql` 전체를 실행합니다.
3. `register_trip.sql`의 여행 ID와 이메일을 확인한 뒤 실행합니다.
4. `trip-config.js`의 Supabase 프로젝트 URL, publishable key, 여행 ID와 편집자 이메일을 확인합니다.

이전에 SQL을 실행했더라도 변경된 `supabase_setup.sql` 전체를 **웹앱 배포 전에** 다시 실행해야 합니다. 스크립트는 기존 일정 데이터를 유지하면서 비공개 `trip_editors` 권한표, revision 충돌 방지, 최근 50개 버전, 공개 `trip-media` Storage 버킷과 편집자 전용 업로드 정책을 설정합니다.

일정에서 사진을 삭제해도 Storage 원본은 즉시 삭제하지 않습니다. 이전 저장 버전에서 사진을 복원할 수 있게 하기 위한 동작입니다. 장기간 사용 후 사용하지 않는 파일을 정리하려면 보관 중인 50개 버전과 현재 문서에서 참조되지 않는 객체만 별도로 확인해 삭제해야 합니다.

publishable key는 정적 웹페이지에 포함해도 되지만, service role key나 편집자 비밀번호는 저장소에 올리지 않습니다.

외부 Supabase JavaScript SDK는 `index.html`에서 고정 버전과 SRI 무결성 해시를 사용합니다. 버전을 올릴 때는 URL과 해시를 함께 갱신하고 테스트를 실행합니다.

## 홈 화면 앱 (PWA)

manifest의 `id`는 생략하여 각 배포 경로의 `start_url`을 앱 식별자로 사용합니다. `id: "./"`를 넣으면 경로가 아닌 도메인 루트로 해석되어 같은 도메인의 다른 앱과 식별자가 겹칠 수 있습니다.

HTTPS로 배포한 주소에서 설치하면 지정된 아이콘과 이름으로 홈 화면에서 실행되며, 지원하는 브라우저에서는 주소창 없는 독립 화면으로 열립니다.

- Android Chrome: 메뉴 → `홈 화면에 추가` → `설치` (브라우저 버전에 따라 메뉴 이름이 다를 수 있습니다).
- iPhone Safari: 공유 → `홈 화면에 추가` → 표시되는 경우 `웹 앱으로 열기`를 켜고 추가합니다.
- 처음에는 인터넷에 연결해 페이지와 최신 공유 일정이 모두 열린 뒤 사용하세요. 설치만으로 모든 여행 사진이 오프라인 저장되는 것은 아닙니다.
- 온라인에서는 화면·설정 파일을 먼저 갱신하고, 네트워크 실패 또는 4초 동안 응답이 없으면 저장된 파일로 앱을 엽니다. 공유 일정 조회도 8초로 제한하여 응답 없는 Wi-Fi에서 계속 로딩되지 않게 합니다. 조회 실패 시 일정 데이터는 기존 IndexedDB 공개본에서 읽으며 저장 시점이 표시됩니다. 로컬 공개본이 없으면 기본 일정임을 표시합니다.
- Supabase 인증·일정 API·사진 Storage 요청은 서비스 워커가 캐시하지 않습니다. 로그인, 서버 저장, 외부 지도와 업로드 사진은 인터넷이 필요합니다. 사진까지 확실히 보관하려면 기존 `오프라인 작업본` 내보내기를 사용하세요.
- 일반적인 재연결은 기존 동기화를 사용합니다. SDK까지 로드하지 못한 상태에서 시작했다면 인터넷 연결 후 앱을 다시 열거나 새로고침하세요.
- 캐시는 브라우저의 저장 공간 정리로 삭제될 수 있습니다. 내려받은 작업본과 달리 영구 백업은 아닙니다.

배포 시 `manifest.webmanifest`, `pwa.js`, `sw.js`, `icons/`, `assets/`를 HTML·설정 파일과 함께 포함합니다. 모든 설치 경로는 상대 경로여서 GitHub Pages의 프로젝트 하위 경로에서도 사용할 수 있습니다. 다른 여행에 재사용할 때는 manifest의 이름·설명과 HTML의 `apple-mobile-web-app-title`도 변경하세요.

캐시 목록·전략·아이콘·이미지를 바꿀 때는 `sw.js`의 `VERSION`을 올립니다. 새 서비스 워커는 기존 앱 창을 모두 닫은 후 활성화되며, 편집 중인 창을 강제 새로고침하지 않습니다. SDK 버전을 올리면 HTML의 URL·SRI와 `sw.js`의 `SDK_URL`·`SDK_INTEGRITY`도 함께 맞춥니다. 아이콘은 `icons/icon.svg`와 같은 도형을 `scripts/generate-icons.ps1`로 PNG로 생성했습니다.

선택적인 실제 Chromium 검증은 Playwright와 Chromium이 설치된 환경에서 `node scripts/check-pwa-browser.mjs`로 실행합니다. Playwright 모듈의 절대 경로를 첫 인자로 지정할 수도 있습니다. 검증은 로컬 가상 API를 사용하며 실제 여행 데이터는 변경하지 않습니다.

## 화면 테마

앱 상단의 `화면 테마`에서 `밝게`(기본값), `어둡게`, `기기 설정`을 선택합니다. 로그인 없이 사용할 수 있고, `기기 설정`은 브라우저의 `prefers-color-scheme` 변경을 실시간으로 따릅니다.

- 선택은 현재 브라우저의 localStorage에 배포 디렉터리별 키(`westbound:theme:<경로>`)로 저장됩니다. Chrome과 삼성 인터넷 사이에는 공유되지 않습니다. 여행 데이터·초안·공유 저장·버전 이력에는 포함되지 않습니다.
- 저장소 접근이 차단되거나 용량이 부족하면 화면에 안내하고 이번 실행에만 선택을 적용합니다. 사이트 데이터를 삭제하면 기본값으로 돌아갑니다.
- 테마 초기화 코드와 CSS는 HTML에 포함되어 오프라인에서도 동작합니다. 편집 가능한 작업본은 테마 기능과 원래 배포 경로만 유지하며, 작성자의 테마 선택은 내보내지 않습니다. `file://` 문서의 선택 저장은 브라우저의 로컬 파일 저장소 지원에 따라 달라집니다.
- 인쇄·공유용 사본은 밝은 테마이며 선택 UI를 제거합니다. 앱에서 인쇄할 때도 밝은 색상을 사용합니다.
- 삼성 인터넷의 **웹 콘텐츠 강제 다크모드**가 사이트 색상을 다시 변환하는 경우가 있습니다. 앱은 밝은 모드에 `color-scheme: only light`를 명시하지만, 브라우저 설정을 직접 끄거나 강제 변환 여부를 신뢰성 있게 감지할 수는 없습니다. 선택과 다르게 표시되면 삼성 인터넷 설정 → 웹페이지 보기 및 스크롤 → 다크 모드에서 웹 콘텐츠 강제 다크모드를 끄고 앱을 다시 여세요. 메뉴 이름은 브라우저 버전에 따라 다를 수 있습니다.

새 버전 배포 후 인터넷에 연결한 상태에서 앱을 다시 열면 화면이 갱신됩니다. 열려 있는 앱 창을 모두 닫고 다시 열면 대기 중인 서비스 워커도 활성화됩니다. 저장하지 않은 편집 중에는 강제 새로고침하지 않습니다.

테마 브라우저 회귀 검증: `node scripts/check-theme-browser.mjs [playwright/index.mjs]`. 설치된 Chrome으로 검증하려면 `PWA_BROWSER_CHANNEL=chrome`을 설정합니다. 로컬 테스트 서버와 격리된 브라우저만 사용하고 실제 공유 저장소 요청은 차단합니다. **Galaxy의 삼성 인터넷 설치 앱에서 강제 다크모드 ON/OFF 확인은 별도 수동 검증이 필요합니다.**

## 로컬 검증

입력 동작 브라우저 검증: `node scripts/check-inputs-browser.mjs [playwright/index.mjs]`. 설치된 Chrome에서 데스크톱·모바일 화면의 입력, 취소 불가능한 입력 이벤트, 한글 조합 이벤트, 실행 취소·다시 실행, 저장 데이터, 불러오기, 내보내기와 파일 재실행을 검사합니다. 운영 서버 요청은 차단하며 실제 휴대폰의 키보드 검증은 별도입니다.

Node.js 20 이상에서 다음 명령을 실행합니다.

```bash
npm test
```

이 명령은 회귀 테스트와 현재 여행 설정·이미지 경로·HTML 문법·SQL 핵심 규칙을 검사합니다. CSS 블록을 다시 합쳐야 할 때만 `npm run styles:consolidate`를 사용합니다. GitHub Actions도 `main` push와 pull request에서 같은 검사를 실행합니다.

## 다른 여행에 재사용

화면과 기능은 `index.html`, 여행별 기본값은 `trip-config.js`에 분리되어 있습니다. `trip-config.template.js`를 새 설정 파일로 복사하고 고유한 `tripId`를 사용하면 여행 일수나 도시가 달라도 탭과 일정이 자동 생성됩니다. 자세한 순서는 [다른 여행 재사용 안내](./NEW_TRIP_GUIDE_KO.md)를 참고하세요.

## Git 작업 흐름

WSL 또는 PowerShell 중 한 환경을 기준으로 작업하고, 변경 전후에 상태를 확인합니다.

```bash
git status
npm test
git add index.html trip-config.js trip-config.template.js supabase_setup.sql register_trip.sql README.md NEW_TRIP_GUIDE_KO.md PROJECT_GUIDE_KO.md PROJECT_GUIDE_SUMMARY_KO.md .gitattributes .gitignore .github package.json scripts tests
git commit -m "feat: update travel planner"
git push origin main
```

여행 일정의 실시간 편집 내용은 Supabase에 저장됩니다. GitHub push는 HTML·SQL 등 소스 코드를 갱신할 때만 필요합니다.

## 배포

GitHub 저장소의 **Settings → Pages**에서 `main` 브랜치와 루트(`/`)를 배포 대상으로 설정합니다. 배포 후 생성되는 GitHub Pages 주소로 일정을 공유할 수 있습니다.
