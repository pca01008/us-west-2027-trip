...
- 일별 일정의 편집 모드에서 `일정 시간 추가` 또는 카드의 `시간 설정`을 누르면 시작 시간과 선택적인 종료 시간을 따로 입력합니다. `09:00` 같은 24시간 형식을 사용하며 종료 시간을 비우면 시작만, 둘 다 비우면 미정으로 저장합니다. `오전·오후·미정 등` 방식에서는 자유롭게 시간대를 적을 수 있습니다. KST 등 시간대 표기는 별도로 유지하며 시작·종료에 같은 기준을 적용합니다. 종료가 시작보다 이르면 다음 날 종료로 안내합니다. 적용 전 취소하면 일정은 바뀌지 않고, 적용한 변경은 실행 취소·다시 실행이 가능합니다.
...
- 전체 보기의 `기존 시간표`와 `한눈에 보기 · 초안`을 전환해 같은 일정을 비교할 수 있습니다. 기본값은 기존 시간표입니다. 초안은 모든 날짜 열과 시간축을 압축하고, 상단의 시간 미정·종일·다른 시간대 일정은 일부 이름과 개수로 요약합니다. 날짜 제목을 누르면 해당 날짜의 1일 보기로 이동하고 `전체`로 돌아오면 초안 선택이 유지됩니다. 모바일에서는 색상 표시와 일정 개수로 전체 흐름을 보여주며 날짜나 세로 열을 눌러 상세한 1일 보기를 엽니다.
...
- `오전`, `미정`, `종일` 등 정확한 시각이 없는 일정과 KST 등 목적지 시간대와 다른 일정은 상단에 원래 표기로 모읍니다. 종류는 일정명으로 자동 분류하고, 하단에는 그날 등록된 체크인·숙박과 이동 내역을 요약합니다. 소요 시간이 없는 이동은 미정으로 표시합니다.
